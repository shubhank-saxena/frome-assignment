...
from tweets.serializers import TweetSerializer, TweetAuthorSerializer, TweetHashtagSerializer
...
class RetrievePostsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if keyword is not None:
            return Tweet.objects.select_related('author').prefetch_related('hashtags').filter(
                Q(tweet__icontains=keyword) | Q(hashtags__hashtag__icontains=keyword)
            ).distinct()
...
class HashtagAnalysisView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        if hashtag is not None:
            tweet_objects = Tweet.objects.filter(hashtags__hashtag__iexact=hashtag).order_by('created_at').select_related('author').prefetch_related('hashtags')
